...
os.chdir(r"Dropbox/Collaborations/Campaign Spending/Python")
if __name__ == "__main__":
    global globtemp 
    globtemp =  None #fo debugging is good.
    files = glob.glob("Files/*.txt")
    with codecs.open(files[0],"r","utf-8") as f:
        txt = f.read()
    
    #turn on to see examples    
    examples = False
...
if __name__ == "__main__":
    files = glob.glob("Files/*.txt")
    with codecs.open(files[45],"r","utf-8") as f:
        txt = f.read()
        self=TemplateTester(txt)
        print self.document.matchstats
    
    
#    template = Template(pullid)
