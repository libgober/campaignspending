...
TIMEOUTTIME = 90 #after 90 seconds give up on parsing.
#%%
constants = dict()
...
def Parse(arr,template,text):
    #parse the document
    dm = DocumentModel(template,text)
    #make it a string
    s = pickle.dumps(dm)
    #a string should be shareable without problems
    arr.value = s
...
class TemplateTester():
    def __init__(self,text):
        ...
        if re.search("\$\s*\d+",text):
            ...
            self.successcount = 0
            arr = mp.Array(c_char,100*len(text))

            #iterate over each template
            #eventually we will want to stop parsing once we reach success
            for template in constants["templates"]:
                #run the Document model in a separate process
                #to ensure timeout on long running code
                p = mp.Process(target=Parse,args=(arr,template,text))
                p.start()
            
                p.join(TIMEOUTTIME)
                
                if p.is_alive():
                    print "template", template, "taking too long, moving on"
                    p.terminate()
                    p.join()
                
                #now check if we have anything in the queue
                new_document = pickle.loads(arr.value)
