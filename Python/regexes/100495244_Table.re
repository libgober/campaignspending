...
(?(DEFINE) #start define block, all naems in here should be lower case, all matches are upper
    ...
    (?<linechar>(?:(?!^\d|\Z)(?!Order\s+Lines\s+Total).))#a character between lines is any one not follwed by a new line and immediately thereafter a digit or the end of character
    (?<datetype> #
      \b(?:0?[1-9]|1[012])
      [/.-]
      (?:0?[1-9]|[12][0-9]|3[01])
...
###Regex match begins now ####
    ...
    #(?&linechar)* comment this out to not go further than the last match for our metatext 